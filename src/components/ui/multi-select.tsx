...
>(({ className, children, ...props }, forwardedRef) => {
  ...
  return (
    <PopoverPrimitive.Trigger ref={forwardedRef as any} asChild>
      ...
      >
        {children}
        <Icons.chevronDown aria-hidden className="size-4 ml-1 shrink-0" />
      </div>
        ...
        >
          {renderItems.map((value) => {
            ...
            const el = (
              <Badge
                key={value}
                className="group/multi-select-badge cursor-pointer rounded-lg pr-1.5 gap-1 h-7"
                onClick={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
              ...
              >
                <Avatar className="h-5 w-5">
                  <AvatarImage
                    src={`/images/avatar/person/${content.replace(
                      /\s/g,
                      ""
                    )}.png`}
                  />
                  <AvatarFallback>{content[0]}</AvatarFallback>
                </Avatar>
                <span>{child}</span>
  ...
  return (
    <PopoverPrimitive.Portal forceMount>
      <PopoverPrimitive.Content
        ...
        collisionPadding={10}
        className={cn(
          'bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 w-full rounded-3xl p-0 shadow-md outline-none',
        )}
        style={
          {
      ...
      >
        <Command
          className={cn(
            'max-h-96 w-full rounded-3xl min-w-[var(--radix-select-trigger-width)] px-1',
            className,
          )}
    ...
    return (
      <CommandItem
        ...
        className={cn(
          ...
          selected && buttonVariants({ variant: 'secondary' }), "py-0 px-2 h-full flex items-center text-noble-black-100 active:text-noble-black-100 hover:text-noble-black-100 ",
          className,
        )}
        disabled={disabled}
      ...
      >
        ...
        {selected ? <Icons.check className="ml-auto size-4 shrink-0" /> : null}
      </CommandItem>
    )
  },
)
